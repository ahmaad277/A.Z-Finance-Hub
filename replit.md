...
- **Investment Management:** Displays investments with status-based color coding, payment progress, and real-time ROI. Features three view modes: (1) Ultra-Compact - smallest single-line strip showing Platform+Status, Number only (fallback to name if no number), APR (blue/chart-1) + ROI (green/chart-2) with larger text-xs fonts, and Face Value; (2) Compact - ultra-dense label-free layout with minimal 2px spacing, displaying two tightly-packed rows of color-coded values only: Center column (APR blue/chart-1 top, ROI green/chart-2 bottom), Left column (Face Value top, Expected Profit green/chart-2 bottom) - no text labels for maximum information density; (3) Expanded - two-section structure with exact Compact copy at top (same 3-column grid, label-free values, 2px spacing) plus organized additional details below (payment progress with green bar, timeline with start/end dates, action buttons, payment schedule manager) separated by border for clear visual hierarchy. View mode controls positioned in page header beside title for easy access.
...
- **Platform Fee Management System:** Automatic profit deduction based on platform-specific fees. Each platform has configurable `feePercentage` (0-99%) and `deductFees` flag (enabled by default). Investment dialog calculates and displays net profit: Net Profit = Gross Profit - (Gross Profit × Fee %). Fee breakdown appears in calculated metrics summary only when applicable, with dynamic 4-column layout. **System now stores NET PROFIT (after fee deduction) in `totalExpectedProfit` as the official value.** For display purposes, gross profit is reverse-calculated: Gross = Net / (1 - Fee%). Division-by-zero protection guards against fees ≥100%. Full bilingual support with fee percentage input using `inputMode="decimal"` for mobile optimization. **Important Limitation:** Legacy investments created before v1.0.1 may store gross profit; manual review/editing may be needed for accurate calculations when platform fees apply.
...
- Platform Fee Management with Net Profit Storage: Added configurable platform fees (0-99%) with automatic net profit calculation. **System now stores NET PROFIT (after fee deduction) as the official `totalExpectedProfit` value**, reversing previous behavior that stored gross profit. Investment dialog auto-calculates net profit, displays fee breakdown when applicable, and includes division-by-zero protection for fees ≥100%. Includes important limitation: legacy investments may require manual review
