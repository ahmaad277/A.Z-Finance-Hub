...
- **Investment Management:** Displays investments with status-based color coding, payment progress, and real-time ROI. Features three view modes: (1) Ultra-Compact - smallest single-line strip showing Platform+Status, Number only (fallback to name if no number), APR (blue) + ROI (green) with larger text-xs fonts, and Face Value; (2) Compact - taller 3-column grid layout (2x height) with Right column (Platform+Status+Duration top, Number+Name bottom), Center column (APR green top, ROI blue bottom), Left column (Face Value top, Expected Profit green bottom); (3) Expanded - full details with timeline and metrics. View mode controls positioned in page header beside title for easy access.
