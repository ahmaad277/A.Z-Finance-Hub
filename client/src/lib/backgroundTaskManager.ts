...
export async function runBackgroundTasksOnce(): Promise<void> {
  ...
  try {
    ...
    if (hasSuccess) {
      // Only mark as run if at least one task succeeded
      // This allows retry on next navigation if both tasks failed
      markAsRun();
      ...
      // Both tasks failed - don't mark as run to allow retry
      if (import.meta.env.DEV) {
        console.error('[BackgroundTaskManager] All background tasks failed:', {
